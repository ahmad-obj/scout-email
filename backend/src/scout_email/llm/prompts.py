...
_TASK_INSTRUCTIONS = {
    ...
    "critic": (
        ...
        "Return a structured assertion audit for every material prospect-specific fact or inference and every material "
        "WEBERAISE self-claim. For each audited assertion, quote the body assertion, identify the corresponding ledger "
        "claim when one exists, cite only the evidence IDs actually supporting that ledger claim, and classify the wording "
        "as ENTAILED, REASONABLE_INFERENCE, SEMANTIC_EXPANSION, or UNSUPPORTED. A semantic expansion is wording that says "
        "something stronger, more specific, or broader in scope than the cited evidence supports; classify it as "
        "SEMANTIC_EXPANSION rather than treating it as a harmless paraphrase. Mark a material prospect assertion as "
        "uncatalogued by leaving ledger_claim null; do not silently excuse uncatalogued prose. For WEBERAISE self-claims, "
        "supply a short exact quote from company context when the claim is supported; do not manufacture a supporting quote. "
        "Set coverage_complete true only after all material assertions have been audited. List generic agency abstractions "
        "or AI-marketing phrases explicitly in copy_abstractions. Do not assume the claim ledger is exhaustive: identify any "
        "material prospect-specific factual or inferential statement in the prose that is uncatalogued, unsupported, "
        "overstated, or inconsistent with its evidence. Perform a semantic entailment check for every claim: wording must "
        "not be stronger, more specific, broader in scope, or use different entities or nouns than the cited evidence "
        "supports. Check quantities, entities, scope, and nouns rather than treating an evidence ID as sufficient by itself. "
        "Modal wording such as may or could does not rescue an unsupported audience, persona, or customer behavior. REWRITE "
        "or REJECT invented audience segments, fake compliments, unsupported customer behavior, unsupported technical "
        "architecture or ordering-flow assumptions, invented metrics or consequences, misleading certainty, or unsupported "
        "comparable-client experience. Audit every WEBERAISE self-claim against the supplied company context and reject "
        "invented specialization or capability. Treat generic agency abstractions and AI-marketing language as quality "
        "defects; prefer plain, specific language tied to the evidence. Apply supplied rejected patterns as concrete examples "
        "of language that requires rewriting. APPROVE only when the complete email is specific, natural, concise, "
        "semantically faithful, evidence-disciplined, and its structured assertion audit is complete."
    ),
}
